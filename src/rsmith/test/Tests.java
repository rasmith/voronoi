...
public class Tests {
	...
	public static void testCircleEvent() {
		...
		for (int i = 0; i < 100; i++) {
			Point2D pi = new Point2D.Double(NumberUtils.randomNumber(0, 10),
					NumberUtils.randomNumber(0, 1000));
			Point2D pj = new Point2D.Double(NumberUtils.randomNumber(11, 20),
					NumberUtils.randomNumber(0, 1000));
			Point2D pk = new Point2D.Double(NumberUtils.randomNumber(21, 30),
					NumberUtils.randomNumber(0, 1000));
			...
			CircleEvent event = CircleEvent.createCircleEvent(sweepY, si, sj,
					sk);
			if (event != null) {
				Point2D c = event.getCenter();
				double r = event.getRadius();
				if (r != c.distance(pi)) {
					System.out.println("Radius inconsistency, r=" + r
							+ ",c.distance(pi)=" + c.distance(pi));
					return;
				}
				...
				if ((d1 - d2) > eps || (d1 - d3) > eps || (d1 - d4) > eps
						|| (d2 - d3) > eps || (d2 - d4) > eps
						|| (d3 - d4) > eps) {
					System.out.println("Distance inconsistency, d1=" + d1
							+ ", d2=" + d2 + ", d3=" + d3 + ", d4=" + d4);
					return;
				}
			} else {
				...
				if (eventY <= sweepY) {
					System.out.println("Event inconsistency: eventY=" + eventY
							+ ", sweepY=" + sweepY);
					return;
				}
				double x = c.getX();
	...
	public static void testSolveRoots() {
		...
		if (roots != null) {
			...
			double y2 = q.eval(x2);
			if (Math.abs(y1) > eps) {
				System.out.println("Solve failed, x1=" + x1 + ",y1=" + y1
						+ ",q=" + q);
				return;
			}
			if (Math.abs(y2) > eps) {
				System.out.println("Solve failed,x2=" + x2 + ",y2=" + y2
						+ ",q=" + q);
				return;
			}
		}
		...
		for (int i = 0; i < 100; i++) {
			...
			for (int j = 0; j < 100; j++) {
				...
				if (Math.abs(d1 - d2) > eps) {
					System.out
							.println("Quadratic distance check failed: q.distance(l)="
									+ d1 + ",q.distance(p)=" + d2);
					return;
				}
			}
	...
	public static void testBisector() {
		...
		for (int i = 0; i < 100; i++) {
			...
			for (int j = 0; j < 100; j++) {
				...
				if (Math.abs(d1 - d2) > eps) {
					System.out.println("Bisector distance check failed: p=" + p
							+ ",q=" + q + ", q.distance(p)=" + d1
	...
	public static void testBreakPoint() {
		...
		for (int i = 0; i < 100; i++) {
			...
			if (bpos.getX() >= cpos.getX()) {
				System.out.println("bpos.x < cpos.x should hold, bpos=" + bpos
						+ ",cpos=" + cpos + "\n p=" + p + ",q=" + q);
				error = true;
			} else {
				double x1 = NumberUtils.randomNumber(bpos.getX() - 1000, bpos
						.getX()
						- eps);
				double x2 = NumberUtils.randomNumber(bpos.getX() + eps, cpos
						.getX()
						- eps);
				double x3 = NumberUtils.randomNumber(cpos.getX() + eps, cpos
						.getX() + 1000);
				...
				if (y1p >= y1q) {
					System.out.println("y1p < y1q should hold: y1p=" + y1p
							+ ",y1q=" + y1q);
					error = true;
				}
				if (y2q >= y2p) {
					System.out.println("y2q < y2p should hold: y2q=" + y2q
							+ ",y2p=" + y2p);
					error = true;
				}
				if (y3p >= y3q) {
					System.out.println("y3p < y3q should hold:y3p=" + y3p
							+ ",y3q=" + y3q);
					error = true;
				}
			}
